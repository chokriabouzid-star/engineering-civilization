...
### تحديث Phase 3 (2026-07-29): سد فجوة `compiler.rs`

اكتُشف بعد إغلاق Phase 2 أن `crates/ec-sandbox/src/compiler.rs` احتوى 5 اختبارات تستدعي Docker مباشرة عبر `RustSandboxCompiler::compile_and_run()` دون أي وسم `cfg_attr`. لم تظهر هذه الفجوة في CI السابق لأن runners تحوي Docker متاحاً دوماً — فالاختبارات كانت تنجح صدفة، لا لأنها محمية. اكتُشفت الفجوة عند تشغيل يدوي في بيئة بلا Docker (فشلت 5 اختبارات كاشفة الثغرة).

**الأسماء الخمسة المُصلَحة:**
- `compiles_hello_world`
- `fails_on_syntax_error`
- `runs_multiple_times`
- `deterministic_output_has_same_hash`
- `network_access_fails_inside_container`

طُبِّق نفس نمط `#[cfg_attr(not(feature = "docker_tests"), ...)]`. العدد الإجمالي لـ `docker_tests`: **44 → 49**. `slow_tests` بلا تغيير (2). المجموع الكلي المكتشف: **691 → بلا تغيير** (فقط توزيع مختلف بين passed/ignored بلا features).

**تحديث موازٍ في `phase2_verify.sh`:** استُبدل التحقق بأرقام مطبوعة يدوياً بتحقق مشتق من طول قائمة الأسماء المصنَّفة (`${#DOCKER_TEST_NAMES[@]} + ${#SLOW_TEST_NAMES[@]}`). هذا يضمن:
- **اكتشاف تلقائي** لأي فجوة مستقبلية مشابهة (اختبار Docker جديد بلا تصنيف)
- **استقلالية** عن نمو المشروع بأي اختبارات سريعة أخرى (لا تكسر الحارس)

**الدرس المنهجي:** فحص الملفات الستة الأصلية كان مركّزاً على أسماء دلالية (`hardened.rs`, `docker.rs`, `week*_gate.rs`) لكن `compiler.rs` بدا "بريئاً" بالاسم رغم أنه في نفس المجلد. **الدرس:** لا يكفي فحص الملفات التي تحمل أسماء واضحة — يجب فحص كل ملف يستورد `DockerRunner` أو أي API يعتمد على Docker.
