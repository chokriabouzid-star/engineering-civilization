...
use crate::proposal::{
    ConstitutionalProposal, ProposalOrigin, ProposedChange, SystemTrigger,
};
...
pub fn propose_from_drift(report: &DriftReport) -> Option<ConstitutionalProposal> {
    match &report.recommended_action {
        ec_memory::DriftAction::HumanIntervention { reason } => {
            Some(ConstitutionalProposal::new(
        ...
        ec_memory::DriftAction::ReviewConstitution { reason } => {
            Some(ConstitutionalProposal::new(
                ProposalOrigin::System {
                    trigger: SystemTrigger::DriftDetected {
                        angle_degrees: report.drift_angle_degrees,
