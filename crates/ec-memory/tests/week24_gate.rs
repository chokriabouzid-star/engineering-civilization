...
use ec_memory::{
    ...
    DriftAction, DriftClassification, HistoricalDriftAnalyzer,
};
...
fn t01_insufficient_data_when_memory_too_small() {
    ...
    let r = a.analyze();
    assert!(
        matches!(r.classification, DriftClassification::InsufficientData { .. }),
        "expected InsufficientData, got {:?}", r.classification
    );
...
fn t02_stable_when_identical_vectors() {
    ...
    assert!(
        ...
        "expected Stable, got {:?}", r.classification
    );
    assert!(r.drift_angle_degrees < 1.0,
        "angle should be ~0°, got {:.2}°", r.drift_angle_degrees);
}
...
fn t03_value_shift_detected_when_priorities_change() {
    ...
    assert!(
        ...
        "expected drift > 10°, got {:.1}°", r.drift_angle_degrees
    );
    assert!(
        !matches!(r.classification, DriftClassification::Stable),
...
fn t04_corruption_detected_when_rejections_increase() {
    ...
    assert!(
        ...
        "expected Corruption, got {:?}", r.classification
    );
...
fn t06_drift_angle_in_valid_range() {
    ...
    let a = HistoricalDriftAnalyzer::new(&g, 10, 10);
    let r = a.analyze();
    assert!(r.drift_angle_degrees >= 0.0,
        "angle must be >= 0, got {}", r.drift_angle_degrees);
    assert!(r.drift_angle_degrees <= 180.0,
        "angle must be <= 180, got {}", r.drift_angle_degrees);
}
...
fn t07_human_intervention_for_large_shift() {
    ...
    assert!(
        ...
        "expected > 45°, got {:.1}°", r.drift_angle_degrees
    );
    assert!(
        matches!(r.recommended_action, DriftAction::HumanIntervention { .. }),
        "expected HumanIntervention, got {:?}", r.recommended_action
    );
}
...
fn t09_empty_memory_returns_insufficient_data() {
    ...
    assert!(
        matches!(
            r.classification,
            DriftClassification::InsufficientData { available: 0, required: 10 }
        ),
        "expected InsufficientData{{0, 10}}, got {:?}", r.classification
    );
}
...
fn t10_week24_gate_complete() {
    ...
    assert!(!matches!(r2.classification, DriftClassification::InsufficientData { .. }));
    ...
    assert!(matches!(r3.classification, DriftClassification::InsufficientData { .. }));

    println!("✅ Week 24 Gate: PASSED — drift={:.1}°", r2.drift_angle_degrees);
}
