...
fn w32_tier1_higher_security_than_tier4() {
    ...
    assert!(
        t1.fitness.security >= t4.fitness.security + 0.20,
        "security: t1={:.2}, t4={:.2}",
        t1.fitness.security,
        t4.fitness.security
    );
...
fn w32_tier1_higher_coverage_than_tier4() {
    ...
    let t4 = analyze_code_full(TIER4);
    assert!(
        t1.fitness.test_coverage > t4.fitness.test_coverage,
        "coverage: t1={:.2}, t4={:.2}",
        t1.fitness.test_coverage,
        t4.fitness.test_coverage
    );
}
...
fn w32_tier1_higher_maintainability_than_tier4() {
    ...
    let t4 = analyze_code_full(TIER4);
    assert!(
        t1.fitness.maintainability >= t4.fitness.maintainability,
        "maint: t1={:.2}, t4={:.2}",
        t1.fitness.maintainability,
        t4.fitness.maintainability
    );
}
...
fn w32_tier1_higher_reversibility_than_tier4() {
    ...
    let t4 = analyze_code_full(TIER4);
    assert!(
        t1.fitness.reversibility > t4.fitness.reversibility,
        "rev: t1={:.2}, t4={:.2}",
        t1.fitness.reversibility,
        t4.fitness.reversibility
    );
}
...
fn w32_confidence_higher_for_ast_than_estimated() {
    ...
    assert!(
        report.confidence.security >= 0.85,
...
fn w32_confidence_lower_for_heuristic_dimensions() {
    ...
    assert!(
        report.confidence.test_coverage <= 0.60,
        "Coverage heuristic → confidence منخفضة، got: {}",
...
fn w32_tier1_fewer_warnings_than_tier4() {
    ...
    let t4 = analyze_code_full(TIER4);
    assert!(
        t1.warnings.len() <= t4.warnings.len(),
        "t1 warnings={}, t4 warnings={}",
        t1.warnings.len(),
        t4.warnings.len()
    );
}
