...
fn w29_comment_not_penalized() {
    ...
    assert!(security_ast(code) >= 0.95,
        "تعليق 'unsafe' لا يُعاقب عليه، got: {}", security_ast(code));
}
...
fn w29_string_literal_not_penalized() {
    ...
    assert!(security_ast(code) >= 0.95,
        "string containing 'unsafe' should not be penalized, got: {}", security_ast(code));
}
...
fn w29_safe_code_confidence_high() {
    ...
    assert!(r.confidence.security >= 0.90,
        "AST analysis → confidence عالية، got: {}", r.confidence.security);
}
...
fn w29_doc_comment_mitigates_unsafe_fn() {
    ...
    assert!(with_doc > without_doc,
        "with_doc={:.2} > without_doc={:.2}", with_doc, without_doc);
}
...
fn w29_unsafe_confidence_still_high() {
    ...
    assert!(r.confidence.security >= 0.85,
        "confidence in detection should be high, got: {}", r.confidence.security);
}
...
fn week29_gate_complete() {
    ...
    println!("  Unsafe block:    sec={:.2} conf={:.2}",
        unsafe_.fitness.security, unsafe_.confidence.security);
    println!("  Comment only:    sec={:.2} (not penalized)",
        comment.fitness.security);
    println!("  Doc+unsafe fn:   sec={:.2} (mitigated)",
